...
its ID is popped off the stack. The function whose ID is at the top
of the stack is the current function being executed. Each time
a function starts or ends, we write a log with the ID, whether
...
twice, one call executing for 2 time units and another call executing for
1 time unit, the exclusive time is 2 + 1 = 3.
...
So function 0 spends 2 + 4 + 1 = 7 units of total time executing, and function 1 spends 1 unit of total
time executing.
...
Learnings:
- The problem is subtely pushing us to use a stack with a list data struct
- LIFO (single-threaded), meaning that a function at the bottom of the stack can't end
    until the ones in the top have ended
- Careful with boundary inclusion
"""
