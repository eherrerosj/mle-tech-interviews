...
Ashe be in poisoned condition. Now, given the Teemo's attacking ascending time series
towards Ashe and the poisoning time duration per Teemo's attacking, you need to
...
class Solution:
    def findPoisonedDuration(self, timeSeries: List[int], duration: int) -> int:
        if not timeSeries:
            return 0
        sol = duration
        for idx, t in enumerate(timeSeries[1:]):
            sol += min(t - timeSeries[idx], duration)
        return sol


sol = Solution().findPoisonedDuration(timeSeries=[1, 4], duration=2)
print(sol == 4)


sol = Solution().findPoisonedDuration(timeSeries=[1, 2], duration=2)
print(sol == 3)
