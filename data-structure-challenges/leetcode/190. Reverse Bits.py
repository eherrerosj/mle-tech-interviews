...
Learnings:
- int to binary in py is better done with format(), either with {0:b} or {0:032b} to force a int size
- My original solution was int("{0:032b}".format(n)[::-1], 2) and yielded the correct result but
    we need to do it using binary ops
- The proposed solution then is to iterate through every bit from right to left and move it to
    the opposite site. We accumulate the result. Do this until n is 0
"""
...
sol = Solution().reverseBits(n=43261596)
print(sol == 964176192)
